...
final class URLSessionRouterTests: XCTestCase {
    ...
    func testThrowsErrorWhenPerformingTaskWithBadParameters() {
        ...
        // When
        sut.performVoidTask(to: MockEndpoint.badMock) { result in
    ...
    func testThrowsErrorWhenPerformingTaskWithBadParametersAsync() async {
        // Given
        let endpoint = MockEndpoint.badMock
        do {
            // When
    ...
    func testTaskSuccess() {
        ...
        // When
        sut.performTask(to: MockEndpoint.mock, completion: { result, response in
    ...
    func testTaskSuccessAsync() async throws {
        ...
        // When
        let result: ResponseEntry = try await sut.performTask(to: MockEndpoint.mock)
        // Then
    ...
    func testTaskReturnsBadStatusCode() {
        ...
        sut.performTask(to: MockEndpoint.mock, completion: { result, response in
            if let response = response as? HTTPURLResponse {
                ...
                XCTAssertTrue(response.allHeaderFields.isEmpty)
                XCTAssertEqual(response.url, URLSessionRouterTests.baseURL)
                if case let .failure(error) = result, case let .invalidResponse(errorResponse) = error as? NetworkLibraryError {
    ...
    func testTaskReturnsBadStatusCodeAsync() async throws {
        ...
        do {
            // When
            let _: ResponseEntry = try await sut.performTask(to: MockEndpoint.mock)
        } catch {
            if case let .invalidResponse(errorResponse) = error as? NetworkLibraryError, let response = errorResponse as? HTTPURLResponse {
    ...
    func testDataTaskSuccessAsync() async throws {
        ...
        // When
        let result: Data = try await sut.performTask(to: MockEndpoint.mock)
        // Then
    ...
    func testDataTaskFailedAsync() async throws {
        ...
        do {
            // When
            let _: Data = try await sut.performTask(to: MockEndpoint.mock)
        } catch {
            if case let .invalidResponse(errorResponse) = error as? NetworkLibraryError, let response = errorResponse as? HTTPURLResponse {
    ...
    func testTaskThrowsError() {
        ...
        let promise = expectation(description: "response")
        // When
        sut.performTask(to: MockEndpoint.mock, completion: { _, response in
            // Then
